...
% 
% %==========================================================================  
% %% Script to compute fortnightly composite images for the study period
% %--------------------------------------------------------------------------  
...
% compositeFG = NaN(length(lat),length(lon),nYears * nMonths);
% compositeFP = NaN(length(lat),length(lon),nYears * nMonths);
% %--------------------------------------------------------------------------  
...
for y = 1:nYears
    ...
    for m = 1:nMonths
        ...
        tempCED = NaN(length(lat),length(lon),numel(yearMonthIdx));  
        tempSST = NaN(length(lat),length(lon),numel(yearMonthIdx));  
        ...
        for j = 1:numel(yearMonthIdx)
            ...
            % SST data
            filepath = fullfile(dataSST,SSTfiles(yearMonthIdx(j)).name);
        ...
        compositeFG(:,:,timeIndex) = mean(tempCED,3,'omitnan');
        ...
        %----------------------------------------------------------------------
        compositeFP(:,:,timeIndex) = (numFROpix ./ numSSTpix) * 100; % FP = N / C *100;
...
%% Save data:  
%..........................................................................  
save('CEDSST_WholeDomain_TimeSeries_30d.mat','-v7.3',...
